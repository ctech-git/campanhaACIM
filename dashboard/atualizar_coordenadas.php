...
$( document ).ready(function() {
  ...
  $.ajax({
   url:"ControlHost/buscar_gerencias.php",
   method:"POST",
   dataType:"json",
   success:function(data)
   {
     ...
     for(var i=0; i<data.length;i++){
       ...
       for(var k=0; k<nome_estado.length;k++){
          var sigla = nome_estado[k].uf;
          if(sigla == estado){
                var codigo_estado = nome_estado[k].codigo_uf;
              for(var j=0; j<municipios.length;j++){
                var codigo_uf = municipios[j].codigo_uf;
                  if(codigo_uf == codigo_estado){
                      if(cidade== municipios[j].nome){
                        data[i].latitude = municipios[j].latitude.toString();
                        data[i].longitude = municipios[j].longitude.toString();

                        latitude = municipios[j].latitude.toString();
                        longitude= municipios[j].longitude.toString();
       ...
       $.ajax({
        url:"ControlHost/atualizar_gerencias_latitude.php",
        method:"GET",
        async:false,
        data:{
          id:id,
